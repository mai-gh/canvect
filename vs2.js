...
ctx.lineWidth = 5;
const funcQ = [];
const getStrokeUnderCursor = () => { for (const s of funcQ) if (ctx.isPointInStroke(s.path, cursor.x, cursor.y)) return s };
const deselectAll = () => { for (const s of funcQ) s.selected = false };
const getAllSelected = () => {
  const selArr = [];
  for (const s of funcQ) if (s.selected) selArr.push(s); 
  return selArr;
};
    ...
    if (clickCounter === 1) {
      ...
      if (getAllSelected().length === 1) {
        ...
        if (ctx.isPointInPath(ps.startBox, cursor.x, cursor.y)) {
          ...
          ps.sx = () => cursor.x + xdiff / 2;
          ps.sy = () => cursor.y + ydiff / 2;
          ps.lx = () => cursor.x - xdiff / 2;
          ps.ly = () => cursor.y - ydiff / 2;
        } else {
          deselectAll();
          clickCounter = 0;
        }
...
const displayStatus = () => {
  ...
  ctx.fillText(statusString, canv.width * 0.81, canv.height * 0.025);
  ctx.restore();
};
